...
The v2 `POST /search`, `/scrape`, `/parse`, and `/map` routes may return an optional top-level `agent_hints: string[]`. Each response contains at most one result-dependent cross-endpoint suggestion. Existing data, errors, warning fields, and HTTP statuses remain unchanged.
